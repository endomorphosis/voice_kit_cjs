...
async function initializeUI() {
  ...
  try {
    ...
    const missingElements = Object.entries(state.elements).filter(([_, value]) => !value).map(([key]) => key);
...
documentReady().then(() => {
  ...
  initializeUI().then(() => Promise.all([initASR(), checkConnection()])).then(() => {
...
async function toggleRecording() {
  try {
    ...
    console.error('Recording error:', error);
    const errorMessage = error.name === 'NotAllowedError' ? 'Microphone access denied. Please allow microphone access.' : `Recording error: ${error.message}`;
